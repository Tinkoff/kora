...
public final class JdbcRepositoryGenerator implements RepositoryGenerator {
    ...
    public MethodSpec generate(ExecutableElement method, ExecutableType methodType, QueryWithParameters query, List<QueryParameter> parameters) {
        ...
        final TypeMirror returnType = (isMono)
            ? ((DeclaredType) method.getReturnType()).getTypeArguments().get(0)
            : method.getReturnType();
        ...
        if (MethodUtils.isVoid(returnType)) {
            ...
            if (returnType.toString().equals(Integer.class.getCanonicalName())
                || returnType.toString().equals("int")) {
                b.addStatement("""
                    return ((int) $T.of(_batchResult)
                              .filter(i -> i == $T.SUCCESS_NO_INFO || i > 0)
                              .count())""", IntStream.class, Statement.class);
            } else if (returnType.toString().equals(Long.class.getCanonicalName())
                       || returnType.toString().equals("long")) {
                b.addStatement("""
                    return $T.of(_batchResult)
                              .filter(i -> i == $T.SUCCESS_NO_INFO || i > 0)
                              .count()""", IntStream.class, Statement.class);
            } else {
            ...
            b.addStatement("_stmt.execute()")
                ...
                .addStatement("return updateCount");
        } else if (returnType.toString().equals(Long.class.getCanonicalName())
                   || returnType.toString().equals("long")) {
            b.addStatement("_stmt.execute()")
                .addStatement("var updateCount = _stmt.getUpdateCount()")
            ...
            b.addStatement("var _updateCount = _stmt.executeLargeUpdate()")
                .addStatement("_telemetry.close(null)")
