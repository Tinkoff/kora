...
public final class OpentelemetryHttpClientTracer implements HttpClientTracer {
    ...
    public CreateSpanResult createSpan(Context ctx, HttpClientRequest request) {
        ...
        builder.setAttribute(SemanticAttributes.HTTP_URL, request.uriResolved());
    ...
    public static HttpClientRequest.Builder inject(HttpClientRequest.Builder builder, @Nullable Span span) {
        if (span == null) {
            return builder;
        }

        W3CTraceContextPropagator.getInstance().inject(span.storeInContext(root()), builder, HttpClientRequest.Builder::header);
        return builder;
    }
}
