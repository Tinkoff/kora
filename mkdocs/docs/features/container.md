...
Аннотация `KoraModule` помечает интерфейс, для которого нужно собрать модуль для текущего модуля компиляции, в него будут помещены все компоненты, помеченные аннотациями `Module` и `Component`. 
...
Например, этот механизм используется для запуска миграций на базе данных при инициализации `JdbcDatabase`:
