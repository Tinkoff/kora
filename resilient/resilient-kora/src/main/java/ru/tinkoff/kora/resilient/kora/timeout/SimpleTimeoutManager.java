...
final class SimpleTimeoutManager implements TimeoutManager {

    private static final Logger logger = LoggerFactory.getLogger(SimpleTimeoutManager.class);
    ...
    SimpleTimeoutManager(TimeoutMetrics metrics, ExecutorService executor, TimeoutConfig config) {
    ...
    public Timeout get(@Nonnull String name) {
        return timeouterMap.computeIfAbsent(name, (k) -> {
            ...
            return new SimpleTimeout(name, config.duration().toNanos(), metrics, executor);
        });
    }
}
