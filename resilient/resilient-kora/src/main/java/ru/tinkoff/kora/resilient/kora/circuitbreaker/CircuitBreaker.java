...
public interface CircuitBreaker {
     ...
     * or throws {@link CircuitBreakerNotPermittedException} if not acquired
     * or fails with exception from {@link Supplier} if it occurred there
     *
     * @param callable to accept to execute for result
     * @param <T>      type of result
     * @return result after {@link #tryAcquire()} was successful or throws {@link CircuitBreakerNotPermittedException}
     * @throws CircuitBreakerNotPermittedException when can't acquire
     */
    <T> T accept(@Nonnull Supplier<T> callable) throws CircuitBreakerNotPermittedException;
     ...
     * @throws CircuitBreakerNotPermittedException when can't acquire
     */
    <T> T accept(@Nonnull Supplier<T> callable, @Nonnull Supplier<T> fallback) throws CircuitBreakerNotPermittedException;

    /**
     * Try to obtain a permission to execute a call. If a call is not permitted, the number of not
     * permitted calls is increased.
     * <p>
     * Throws a CircuitBreakerNotPermittedException when the state is OPEN or FORCED_OPEN. Returns when the
     * state is CLOSED or DISABLED. Returns when the state is HALF_OPEN and further test calls are
     * allowed. Throws a CircuitBreakerNotPermittedException when the state is HALF_OPEN and the number of
     ...
     * the call is cancelled before it is invoked, you have to release the permission again.
     *
     * @throws CircuitBreakerNotPermittedException when CircuitBreaker is OPEN or HALF_OPEN and no further test calls are permitted.
     */
    void acquire() throws CircuitBreakerNotPermittedException;
