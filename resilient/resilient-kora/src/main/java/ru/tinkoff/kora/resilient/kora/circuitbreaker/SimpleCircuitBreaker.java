...
record SimpleCircuitBreaker(
    ...
    SimpleCircuitBreaker(String name, CircuitBreakerConfig.NamedConfig config, CircuitBreakerPredicate failurePredicate, CircuitBreakerMetrics metrics) {
    ...
    @Override
    public void acquire() throws CircuitBreakerNotPermittedException {
        if (!tryAcquire()) {
            throw new CircuitBreakerNotPermittedException(getState(state.get()), name);
        }
    }
